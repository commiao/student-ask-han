...
const ALIASES = [
  ...
  // G-1（第 7 轮）：这行原先**无条件**展开。`登记` 是全库 df=0/6 的泛词（库里一个字都没有），
  // 任何提到"登记"的问句都白送一次 `报到时间` 命中。实测 `几点回宿舍要登记吗`（文档只写门禁
  // 时间、没写要不要登记，属越界）就是这么过的：清完停词剩 4 个原话二元组，`宿舍` 撞上 报到须知
  // 里 Q11「分班分宿舍结果在哪里查」= 1/4，再白捡 `报到时间` = 2/4 = 0.50 正好踩线。
  // 收窄口径与 卡/门口/被褥/几个人 同一处理方式：只在问句确实在问报到注册时才展开。
  ['登记', ['报到时间'], /报到|入学|注册/],           // `入学登记是哪几天` → Q1
  ...
  // ── 第 7 轮 G-2 批次 1（口语探针 B 类误拒）。三条都是**上一轮整批回滚时撤掉的同三个 key**，
  //    这轮带条件重做：落点负例见 cases.neg.md §5（团籍咋交团费/团籍咋注销/团籍档案丢了能补吗/
  //    卖饮品需要健康证吗/饮品热量高吗/插排坏了找谁修），逐条实测过线分数 <0.50 才留下。
  // 展开词的个数刻意压在 2：`团籍档案丢了能补吗` 原话只命中 档案（1/6），再给 2 个别名词就正好 0.50 放行，
  // 所以 `转接` 这个词不能加（它是 Q3 正文词，等于给一切"团籍+丢了/补"类问法白送第三分）。
  ['团籍', ['团总支', '团员关系'], /^团籍(咋|怎么|如何)?办$/],  // `团籍咋办` → Q4（整句匹配：`团籍证明怎么办` 必须仍拒）
  ['饮品', ['奶茶', '瑞幸'], /卖|哪儿有|哪里有|有没有/],        // `有卖饮品的吗` → Q34；只在问"有没有卖"时展开，`饮品热量高吗` 不给
  ['插排', ['插孔', '插座'], /能|够|可以|允许/],                // `插排能用吗` → Q26；`插排坏了找谁修`/`插排哪个牌子好` 无条件可用 → 0.40 拒
  // ── 第 7 轮 G-2 批次 2（A 类误拒：词就在文档里，短问句吃亏在分母小）。──
  // 这三行的展开词只给 **1 个**：A 类问句原话已经命中一个文档词（外卖/水果），再补 1 个就够 2/3、2/4 过线；
  // 给到 2 个就会把同 key 的越界问法（`能点外卖要加钱吗` 5 元组、`买点水果要不要洗` 5 元组）一起抬到 0.5 以上。
  ['能点外卖', ['进校园']],                              // `能点外卖吗` → Q33（"外卖均可入校放到外卖架"，`进校园` 是 Q33 标题原词）
  ['买点水果', ['水果捞']],                              // `想买点水果` → Q35（`水果捞` 是 Q35 答句原词）
  // `床宽` 是泛词形态（"床宽了能换张床吗"文档只答尺寸、不答换床），所以带条件：只在问宽度时展开。
  ['床宽', ['床多大', '宿舍床', '米'], /不宽|多宽|几米/],       // `床宽不宽` → Q27
  // ── 第 10 轮：Q38–Q55 收编后的口语召回。所有展开词都逐字来自新增条目的标题或答案；
  //    涉及“贷款/缴费”“体检/地点”“床尺寸”等相邻主题的键必须带条件，避免给越界问法抬分。──
  ['带什么材料', ['录取通知书', '身份证', '照片']],
  ['需要带哪些材料', ['录取通知书', '身份证', '照片']],
  ['料材', ['材料', '录取通知书', '身份证']],
  ['材料要带', ['录取通知书', '身份证', '照片']],
  ['团组织关系转到哪里', ['团员关系', '怎么转', '重庆工商职业学院']],
  ['助学贷款', ['办理点', '居住地', '现场']],
  ['助学贷宽', ['助学贷款', '办理点', '居住地']],
  ['宿舍怎么安排', ['分班', '结果', '群文件']],
  ['宿舍是怎么安排的', ['分班', '结果', '群文件']],
  ['团支不', ['团支部']],
  ['团委系统', ['团重庆市委', '重庆工商职业学院团委', '电子信息工程学院团总支', '搜索框']],
  ['学圆', ['学院'], /团|总支|转错/],
  ['团关系', ['团组织关系', '自行撤销', '重新转入'], /转错|校团委|团总支/],
  ['团支书身份', ['取消职务', '管理员身份', '高中学校团委']],
  ['高中团干部', ['团支书', '取消职务', '管理员身份']],
  ['网上缴学费', ['掌上迎新', '公共服务', '公众号', '绑定', '页面提示', '首次使用'], /^(?!.*(贷款|绿色通道)).*网上缴学费/],
  ['学费在网上怎么缴', ['掌上迎新', '公共服务', '公众号', '绑定', '页面提示', '首次使用'], /^(?!.*(贷款|绿色通道)).*学费在网上怎么缴/],
  ['网上如何缴', ['掌上迎新', '公共服务', '公众号', '绑定', '页面提示', '首次使用'], /^(?!.*(贷款|绿色通道)).*网上如何缴/],
  ['学费在网上如何缴', ['掌上迎新', '公共服务', '公众号', '绑定', '页面提示', '首次使用'], /^(?!.*(贷款|绿色通道)).*学费在网上如何缴/],
  ['学飞网上', ['学费', '掌上迎新', '公共服务', '绑定']],
  ['掌上迎新', ['网上缴费', '绑定', '页面提示'], /付款|缴|交|学费/],
  ['只做困难认定', ['国家助学金', '新生圆梦', '第一步', '拿不到钱']],
  ['申请资助', ['国家助学金', '新生圆梦', '家庭经济困难认定']],
  ['资住', ['资助', '低保证', '佐证材料']],
  ['村委会', ['村上', '明令禁止', '一律无效'], /贫困|困难|证明|材料/],
  ['村委盖章', ['村上', '明令禁止', '一律无效']],
  ['证明少了', ['班级评议小组', '申请', '佐证材料']],
  ['校内兼职', ['勤工助学', '资助中心', '辅导员通知', '一个岗位']],
  ['检查身体', ['体检', '98', '现金', '微信支付'], /费用|多少钱|收费|缴费|交费/],
  ['开学前不用', ['开学报到后', '活动中心', '体检'], /体检/],
  ['检查身体', ['活动中心', '领取体检单', '4 楼'], /哪里|哪儿|什么地方/],
  ['新生群备注明', ['群备注', '年级', '专业', '姓名']],
  ['专业年级姓名', ['xx年级', '群备注', '群管理', '没有修改', '清理出群', '专业', '姓名']],
  ['不改群备注', ['群备注', '清理']],
  ['群名片', ['群备注', '年级', '专业', '姓名']],
  ['桃园', ['190', '190×80', '80 厘米', '李园', '园区', '床垫', '床上用品', '实际园区']],
  ['竹园', ['194', '80 厘米', '梅园']],
  ['尺村', ['尺寸', '竹园', '194']],
  ['宿舍园区', ['桃园', '李园', '竹园', '梅园', '190', '194']],
  ['床宽', ['80 厘米', '桃园', '李园', '竹园', '梅园'], /90|厘米/],
  ['床铺规格', ['桃园', '李园', '190', '80 厘米']],
  ['宿舍床的尺寸', ['桃园', '李园', '竹园', '梅园', '190', '194']],
  ['宿舍床尺寸', ['桃园', '李园', '竹园', '梅园', '190', '194']],
  ['校园地', ['校园地贷款', '本校没有', '生源地贷款']],
  ['校圆地', ['校园地贷款', '本校没有', '生源地贷款']],
  ['贷款还要缴费', ['生源地贷款', '贷款回执', '短信截图', '绿色通道', '不用在网上缴费']],
  ['办了生源地贷款', ['贷款回执', '短信截图', '绿色通道', '不用在网上缴费']],
  ['有贷款还', ['生源地贷款', '贷款回执', '短信截图', '绿色通道', '不用在网上缴费']],
  ['学校公重号', ['学校公众号', '公共服务', '掌上迎新']],
  ['从哪儿进入', ['学校公众号', '公共服务', '掌上迎新']],
];
...
function itemsOf(payload) {
  ...
  // 一个来源文件也可以只承载一个可引用 Q（例如总体说明末尾的 Q55）。
  // 只把 >=2 当门槛会让单 Q 文件走普通分段回退，丢掉 Q 编号并被 no-citable 门禁过滤。
  if (marks.length >= 1) {
 ...
 */
// 整句只由"版本 + 疑问尾巴"构成才算命中：前缀（什么/当前/现在/你的）与尾巴（是/为 + 什么/多少/如何）
// 各自可选，靠上面 versionOf 的 ≤8 字闸兜住"还带了别的内容"的情况。
// 为什么不写成 includes：`宿舍住宿要登记版本号吗` 这类真问题必须走检索（实测：收窄前它正常拒答，
// 若用 includes 就会被这条常量劫走）。旧写法把每个说法列成并列分支，漏了 `版本号是多少`——
// 尾巴组合是"是+多少"，而列出的只有 `多少` 或 `是什么` 两种，实测它掉进检索回了固定话术。
const VERSION_RE = /^(什么|啥|哪个|哪|当前|现在|你的|你)?版本(号)?(是|为)?(什么|啥|多少|几号|如何)?$/;
...
export function apply(ctx, config) {
  ...
  ctx.tools.register({
    ...
    async execute(args) {
      ...
      // 标题字面覆盖率：问句里的字有多少落在**这条的标题**里。它只在词项分数接近时破平手：
      ...
      for (const c of cand) {
        ...
        c.titleCov = titleCov;
        // 1.5 足以让「宿舍床的尺寸」优先命中标题完整覆盖的 Q54，而不是只因「宿舍」是章节名
        // 就被 Q27 的章节加成压过；仍显著小于明确主题词/章节的主分数。
        c.score = score + titleCov * 1.5 - c.rank * 0.01;
      ...
      // 闭卷承诺的硬不变量（第 6 轮）：ANSWER 至少要有**一条能标注出处**的条目，否则退回拒答。
      // 触发它的是实测到的 `引用=[无]` 形状：Web 端导入 + AI 整理过的群精华消息进了库，这类 payload
      // 没有 `## Q数字.` 结构，itemsOf() 只能把整块当成一条、label 为空——它有正文、能被命中、
      // 会被投喂，却打不出「（指南·章节 第N条 Qn）」。让这种内容当依据等于给出没有出处的回答，
      // 而"每条答案可追溯"是这套机制唯一的可信度支柱，所以宁可拒。
      // 顺带把混在里面的不可标注条目也剔掉：只留有出处的部分进回复。
      // 代价要知道：将来若有文档只写"第N条"不写 Qn，会被这条判成不可标注而拒答——那是切块契约问题，
      // 该回去补结构（或在 itemsOf 里认那种标题），不该靠放宽这条不变量绕过。
      const citable = picked.filter((p) => /\bQ\d+\b/.test(String(p.label ?? '')));
      if (citable.length === 0) {
        return refuse([`via: no-citable`, `tried: ${JSON.stringify(terms)}`]);
      }
      picked = citable;
