...
const ALIASES = [
  ...
  // ── 第 6 轮（口语探针 `probe-spoken.mjs` 量出来的 A 类误拒：词就在文档里，但问句太短、
  //    原话二元组个数就是分母，命中率天然过不了 0.5。修法是给这些**新 key** 配足展开词，
  //    让别名去补分子——`GRAM_MIN` 和"分母只数原话二元组"两条都不动）。
  //    展开词全部逐字抄自 `## Qn` 原文（现查，见 FIXPLAN 工单）；泛词 key 一律带第三元素。
  ['点外卖', ['外卖架', '能进校园', '入校']],                 // `能点外卖吗` → Q33
  ['买点', ['水果捞', '超市', '水果'], /水果|吃/],            // `想买点水果` → Q35；`买点菜自己做饭` 不许被喂活
  ['床宽', ['床多大', '宿舍床', '米']],                       // `床宽不宽` → Q27
  ['插排', ['插孔', '多排插座', '接线'], /插|电|用/],          // `插排能用吗` → Q26
  ['回宿舍', ['门禁', '宵禁', '进不出'], /几点|晚|夜/],        // `几点前回宿舍` → Q21
  ['卫生间', ['卫浴', '独立'], /宿舍|住|间/],                  // `有自己独立的卫生间没` → Q13（库里只有"卫浴"）
  // ── 第 6 轮第二批（B 类词表错配 + 剩下的 A 类）。key 都选**问句里那段口语**，
  //    value 一律是目标条目原文里的词；泛词 key 必须带条件正则，理由同 `门口`/`卡` 那两行。
  ['团籍', ['团员', '团总支', '关系转']],                     // `团籍咋办` → Q4
  ['饮品', ['奶茶', '咖啡', '瑞幸']],                          // `有卖饮品的吗` → Q34
  ['墙上', ['装饰', '床帘', '安装']],                          // `墙上能贴东西吗` → Q17
  ['查身体', ['体检', '空腹', '抽血']],                        // `开学要查身体吗` → Q8
  ['先干', ['先做什么', '报到流程', '报到处']],                 // `去了先干嘛` → Q7
  ['寄东西', ['邮寄地址', '快递', '收件人']],                   // `寄东西地址写啥` → Q12
  ['位置', ['地址', '校区', '导航'], /学校|在哪|地址|怎么走/],  // `学校位置在哪` → Q12；泛词，必须带条件
  ['电饭煲', ['大功率电器', '电器', '吹风机']],                 // `宿舍里能自己带电饭煲煮面吗` → Q22
  ['哪些东西', ['材料', '档案', '证件照']],                     // `要带哪些东西去学校` → Q3
  ['那块', ['银行卡', '一类卡', '激活'], /银行|卡/],            // `银行那块学校怎么弄` → Q37
  ['楼下', ['陪同', '进校', '行李'], /妈|爸|家长|送/],           // `我妈能送到楼下吗` → Q5；无`楼下便利店`类问句被喂活
  ['住宿', ['宿舍', '床位', '几人间', '分配'], /安排|分配|怎么/], // `住宿怎么安排` → Q13/Q15/Q25
