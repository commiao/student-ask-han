...
const ALIASES = [
  ...
  ['登记', ['报到', '报到时间']],                    // `入学登记是哪几天` → Q1
  ['在哪', ['地点', '位置'], /报到|注册|收件|邮寄/],   // `在哪里报到` → Q2；带条件，免得"哪儿洗/在哪儿看"这类被喂进地点条目
  ['什么地方', ['地点', '位置', '在哪里']],            // `办理入学手续在什么地方` → Q2（同 section 里 Q7 是"流程"，靠"地点"把它压下去）
  ['入学手续', ['报到', '报到地点', '材料', '证件']],   // 同上；刻意不给"报到流程"，那个词只会把 Q7 顶上来
  ...
  ['体剑', ['体检']], ['检查身体', ['体检', '空腹', '抽血', '新生', '报到']], // `报到时要体剑吗` / `入学检查身体吗` → Q8
  ...
  ['住几人', ['几人间', '六人间', '四人间', '几人']], ['厕所', ['卫浴', '卫生间']], // `住几个人，有厕所吗` → Q13
  ['房间', ['宿舍']], ['电扇', ['风扇']], ['冷气', ['空调']], // `房间有电扇和冷气吗` → Q14
  ['挑吗', ['选', '互换']], ['铺位', ['床位', '分配']], // `床位不能自己挑吗` / `铺位是怎么定的` → Q15
  ['衣贵', ['衣柜']], ['柜子', ['衣柜', '有衣柜']], ['放衣服', ['衣柜']], // → Q16
  ...
  ['被褥', ['床上', '生活用品', '自带', '学生']],               // `宿舍需要自带被褥不` → Q24（"需要/带"是问法词，被剥掉后只剩脚手架）
